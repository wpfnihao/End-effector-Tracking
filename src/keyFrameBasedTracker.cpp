...
keyFrameBasedTracker::keyFrameBasedTracker()
:numOfPatchScale(10)
,buffSize(20)
,numOfPatches(10)
,numPatches(2)
...
void
keyFrameBasedTracker::track(void)
{
	...
	// TODO: initialization of keyFrames
	std::vector<int> patchID;	
	findClosestPatch(cMo, keyFrames, numPatches, patchID);
	...
	optimizePose(curImg, prePatch, keyFrames, patchID);
...
void
keyFrameBasedTracker::updateDataBase(void)
{
...
void 
keyFrameBasedTracker::initDataset(void)
{
}

void 
keyFrameBasedTracker::pushDataIntoBuff(patch& patchData)
{
...
bool 
keyFrameBasedTracker::getDataFromBuff(patch& patchData)
{
	bool isNewData;
	omp_set_lock(&buffLock);
	if (!buff.empty())
...
void 
keyFrameBasedTracker::updataDataset(patch& patchData)
{
...
void 
keyFrameBasedTracker::processKeyPatch(patch& patchData)
{
	...
	int 
keyFrameBasedTracker::findPatchScale(int faceID)
{
	patch p;
	...
	void 
keyFrameBasedTracker::findPatchScale(patch& patchData)
{	
...
//void
//keyFrameBasedTracker::genOrgScalePatch(patch& patchData)
//{
//	// set the confidence
...
keyFrameBasedTracker::genOrgScalePatch(patch& patchData)
{
	...
	void
keyFrameBasedTracker::genRestScalePatch(patch& patchData, int scaleID)
{
	...
	inline void
keyFrameBasedTracker::interpolate(patch& patchData, int scaleID)
{
	cv::resize(
	...
	bool
keyFrameBasedTracker::findConfidence(int scaleID, int& patchID, patch& patchData)
{
	std::vector<int> IDs;
	int count = 0;
	...
	void
keyFrameBasedTracker::superResolution(int scaleID, int patchID, patch& patchData)
{
	// DEBUG only
	...
	void
keyFrameBasedTracker::refreshDataset(void)
{
	int len = vpMbEdgeTracker::faces.getPolygon().size();
	...
	void
keyFrameBasedTracker::findLight(const cv::Mat& tarLowPatch,const cv::Mat& lowPatch, cv::Mat& light)
{
	// use graph cuts here
...
// FIXME: if necessary, only use the most close scale
	void
keyFrameBasedTracker::findCopyProcessPatch(
		int faceID,
		int patchScale,
...
/* old version back up
void
keyFrameBasedTracker::projPatch(vpHomogeneousMatrix pose, int id, std::list<patch>& dataPatches)
{
	float rate = getRate(maxDownScale, numOfPatchScale);
	int r = rate * rows,
		c = rate * cols;
...
void
keyFrameBasedTracker::deepCopyPatch(std::list<patch>& dataPatches, patch& src)
{
	patch p;
...
void
keyFrameBasedTracker::obtainPatch(int faceID, patch& p)
{
...
vpMatrix 
keyFrameBasedTracker::getVirtualCam(void)
{
	float rate = getRate(maxDownScale, numOfPatchScale);
	vpMatrix K = cam.get_K();
	K[0][0] *= rate;
...
//
//inline void
//keyFrameBasedTracker::projPoint(
//		const vpMatrix& invK, 
//		const vpMatrix& invP, 
...
/*
   float
   keyFrameBasedTracker::calcDepth(
...
inline cv::Point
keyFrameBasedTracker::meanPoint(const std::vector<cv::Point>& p)
{
	cv::Point cvp;
...
void 
keyFrameBasedTracker::optimizePose(cv::Mat& img, dataset_t& prePatch, keyFrameDataset_t& dataPatches, std::vector<int> patchID)
{
	p_cMo = cMo;
	res = getPose(I, img, prePatch, dataPatches, patchID);
}

void
keyFrameBasedTracker::deepCopyPrePatch(patch& src, patch& p)
{
	p.patchScale 	= src.patchScale;
...
// TODO: complete this function
	bool
keyFrameBasedTracker::isKeyFrame(void)
{
	float th1 = 0.001;
	...
	void 
keyFrameBasedTracker::initialization(cv::Mat& src, std::string config_file, std::string modelName, std::string initName)
{
...
void
keyFrameBasedTracker::initFaceScaleInfo(void)
{
	// for faceSizes
	initFaceScaleSize();
	...
	void
keyFrameBasedTracker::initFaceScaleDepth(void)
{
	int i = 0;
	for (std::vector<vpMbtPolygon*>::iterator itr = vpMbEdgeTracker::faces.getPolygon().begin(); itr != vpMbEdgeTracker::faces.getPolygon().end(); ++itr)
	{
...
void
keyFrameBasedTracker::initFaceScalePose(void)
{
	int i = 0;
	for (std::vector<vpMbtPolygon*>::iterator itr = vpMbEdgeTracker::faces.getPolygon().begin(); itr != vpMbEdgeTracker::faces.getPolygon().end(); ++itr)
	{
	...
	void
keyFrameBasedTracker::initFaceScaleSize(void)
{
	float rate = getRate(maxDownScale, numOfPatchScale);
...
void
keyFrameBasedTracker::initFaceScaleVirtualCam(void)
{
	float rate = getRate(maxDownScale, numOfPatchScale);
	for (size_t i = 0; i < vpMbEdgeTracker::faces.getPolygon().size(); i++)
	{
...
vpMatrix
keyFrameBasedTracker::scaleCam(vpMatrix K, float rate)
{
	K[0][0] /= rate;
	K[0][2] /= rate;
	K[1][1] /= rate;
	...
	inline float 
keyFrameBasedTracker::pointDistance3D(const vpPoint& p1, const vpPoint& p2)
{
	float x1 = p1.get_oX();
	float y1 = p1.get_oY();
	float z1 = p1.get_oZ();
		...
		)
{
	...
	void
keyFrameBasedTracker::prepCalcDepth(
		vpPoint*  vp, 
		vpMatrix& A, 
		vpMatrix& b,
		float* coefficient)
...
void
keyFrameBasedTracker::findMinMax(std::vector<cv::Point> corners, cv::Point& lu, cv::Point& rb)
{
	lu = corners[0];
	rb = corners[0];
...
{ 
	...
	void 
keyFrameBasedTracker::findStableFeatures(
...
#pragma omp parallel for num_threads(2)
	...
	void
keyFrameBasedTracker::findStableFeaturesWithRate(
		const std::vector<float>& 	 	fErr, 
		const std::vector<uchar>& 	 	fStatus, 
		std::vector<bool>& 			 	finalStatus, 
		const std::vector<cv::Point2f>& corners,
...
void
keyFrameBasedTracker::findClosestPatch(
		vpPoseVector& 		vp, 
		std::vector<int>& 	id, 
		std::vector<int>& 	idx, 
...
#pragma omp parallel for num_threads(4)
	...
	inline int 
keyFrameBasedTracker::findMinCost(float tar, int pos, const cv::Mat& img , int rows, int cols, int ws)
{
	int r = pos / cols;
	int c = pos % cols;
	cv::Mat subMat = img(
...
double
keyFrameBasedTracker::computeVVS(
...
double
keyFrameBasedTracker::getPose(const vpImage<unsigned char>& I, cv::Mat& img, dataset_t& prePatch, keyFrameDataset_t& dataPatches, std::vector<int> patchID)
{
	double res;
	...
	vpPoseFeatures pf;
	trackPatch(pf, img, prePatch, dataPatches, patchID);	
	...
	res = computeVVS(nbInfos, I, pf, w_mbt, w_klt);
 ...
 */
inline double
keyFrameBasedTracker::pointDistance2D(const cv::Point& p1, const cv::Point& p2)
{
	double dx = p1.x - p2.x;
	double dy = p1.y - p2.y;
...
void
keyFrameBasedTracker::trackPatch(vpPoseFeatures& featuresComputePose, cv::Mat& img, dataset_t& prePatch, keyFrameDataset_t& dataPatches, std::vector<int> patchID)
{
...
#pragma omp section
		{
			// for keyFrames
			vpMatrix invK = cam.get_K().inverseByLU();
			for (int np = 0; np < numPatches && np < patchID.size(); np++)
			{
				int id = patchID[np];
				cv::Mat& keyImg = dataPatches[id].frame;
				vpHomogeneousMatrix& keyPose = dataPatches[id].pose;
				std::vector<patch>& keyPatches = dataPatches[id].orgPatches;

				std::vector<cv::Mat> keyPyramid;
				cv::buildOpticalFlowPyramid(keyImg, keyPyramid, cv::Size(21, 21), 3);
				// for keyFrames
				for (size_t i = 0; i < vpMbEdgeTracker::faces.getPolygon().size(); i++)
					if (isVisible[i] && !keyPatches[i].orgPatch.empty())
					{
						patch& pp = keyPatches[i];
						// restore the patch to the image size
						cv::Mat orgPatch = cv::Mat::zeros(cv::Size(cols, rows), CV_8UC1);
						cv::Mat mask 	 = cv::Mat::zeros(cv::Size(cols, rows), CV_8UC1);
						pp.orgPatch.copyTo(
						...
						std::vector<uchar> fStatus, bStatus;
						// forward-backward track
						cv::calcOpticalFlowPyrLK(keyPyramid, cPyramid, corners, cFeatures, fStatus, fErr, cv::Size(21, 21), 3, cv::TermCriteria(cv::TermCriteria::COUNT+cv::TermCriteria::EPS, 20, 0.03));
						cv::calcOpticalFlowPyrLK(cPyramid, keyPyramid, cFeatures, bFeatures, bStatus, bErr, cv::Size(21, 21), 3, cv::TermCriteria(cv::TermCriteria::COUNT+cv::TermCriteria::EPS, 20, 0.03));
						std::vector<bool> finalStatus(corners.size());
						float th = 0.5 * 0.5; // the threshold of distance is 0.5
						float rate = 0.2;
						findStableFeatures(finalStatus, corners, bFeatures, fStatus, bStatus, fErr, th, rate);
...
void
keyFrameBasedTracker::trackPatchOrb(vpPoseFeatures& featuresComputePose, cv::Mat& img, dataset_t& prePatch, dataset_t& dataPatches)
{
	//vpMbEdgeKltTracker::track(I);
...
void
keyFrameBasedTracker::trackPatchSIFT(vpPoseFeatures& featuresComputePose, cv::Mat& img, dataset_t& prePatch, dataset_t& dataPatches)
{
	//vpMbEdgeKltTracker::track(I);
...
void
keyFrameBasedTracker::trackPatchSURF(vpPoseFeatures& featuresComputePose, cv::Mat& img, dataset_t& prePatch, dataset_t& dataPatches)
{
	//vpMbEdgeKltTracker::track(I);
...
void
keyFrameBasedTracker::findCorner(cv::Mat& mask, cv::Point& upLeft, cv::Point& rightbottom)
{
	upLeft.x 		= mask.cols;
	upLeft.y 		= mask.rows;
	rightbottom.x 	= 0;
...
inline void
keyFrameBasedTracker::genMask(cv::Mat& curMask, cv::Point& upLeft, cv::Point& rightbottom, int erodeSize)
{
...
void
keyFrameBasedTracker::findClosestPatch(vpHomogeneousMatrix& cMo, keyFrameDataset_t& keyFrames, int numPatches, std::vector<int>& id)
{
	if (keyFrames.empty())
		return;
	...
	vpPoseVector vp;
	vp.buildFrom(cMo);
	std::vector<float> diff(keyFrames.size());
	for (size_t i = 0; i < keyFrames.size(); i++)
	{
		vpPoseVector p;
		p.buildFrom(keyFrames[i].pose);
		diff[i] = fabs(p[0] - vp[0])
			+ fabs(p[1] - vp[1])
			+ fabs(p[2] - vp[2]);
		//DEBUG only
		//std::cout<<"distance is  ------------------------------------- "<<diff[i]<<std::endl;
		//end of DEBUG
	}
